    ...
    <section className={styles.gallery} aria-label={title}>
          ...
          <div className={styles.stageFrame}>
            {items[activeIndex]?.type === 'video' ? (
              <div className={styles.stageVideoPoster}>
                ...
                <span className={styles.playBadge} aria-hidden="true"><PlayOutline size={32} /></span>
              </div>
            ) : (
            ...
            >
              {item.type === 'video' ? (
                <div className={styles.thumbVideoWrap}>
                  ...
                  <span className={styles.thumbPlayBadge} aria-hidden="true"><PlayOutline size={16} /></span>
                </div>
              ) : (
      ...
      {lightboxOpen && (
        <div
          ...
          role="dialog"
        >
          <button aria-label="Close lightbox" className={styles.lightboxClose} onClick={closeLightbox} ref={closeButtonRef} type="button">
            <Close size={20} />
          </button>
          <button
            aria-label="Previous"
          ...
          <div className={styles.lightboxMedia}>
            {lightboxItem?.type === 'video' ? (
              <video autoPlay className={styles.lightboxMediaEl} controls poster={lightboxItem.poster} src={lightboxItem.src || undefined} title={lightboxItem.alt} />
            ) : (
              <img alt={lightboxItem?.alt ?? ''} className={styles.lightboxMediaEl} src={lightboxItem?.src} />
            )}
          </div>
          <button
            aria-label="Next"
          ...
          <div className={styles.lightboxFooter}>
            <div className={styles.lightboxMeta}>
              ...
              <span className={styles.lightboxCounter}>{lightboxIndex + 1}&thinsp;/&thinsp;{items.length}</span>
            </div>
              ...
              {items.map((item, i) => (
                ...
                >
                  {item.type === 'video' ? (
                    <div className={styles.thumbVideoWrap}>
                      <img alt="" src={item.poster ?? item.src} />
                      <span className={styles.thumbPlayBadge} aria-hidden="true"><PlayOutline size={14} /></span>
                    </div>
                  ) : (
                    <img alt="" src={item.src} />
                  )}
